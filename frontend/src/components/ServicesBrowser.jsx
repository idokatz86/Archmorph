...
export default function ServicesBrowser() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Service Grid/List */}
      {filtered.length === 0 ? (
        <Card className="flex flex-col items-center justify-center p-16 text-center border-dashed border-2 border-border bg-surface">
          <Server className="w-12 h-12 text-text-muted mb-4 opacity-50" />
          <h3 className="text-lg font-semibold text-text-primary mb-2">No services found</h3>
          <p className="text-sm text-text-muted max-w-sm">
            We couldn't find any services matching your current filters. Try relaxing your search terms.
          </p>
          <button onClick={() => { setSearch(''); setProvider('all'); setCategory('all'); }} className="mt-6 px-4 py-2 bg-cta text-white font-medium rounded-lg hover:bg-cta/90 transition-colors text-sm shadow-sm cursor-pointer">
            Clear Filters
          </button>
        </Card>
      ) : (
