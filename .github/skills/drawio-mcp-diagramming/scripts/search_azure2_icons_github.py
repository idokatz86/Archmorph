#!/usr/bin/env python3
"""
Fallback search for Azure2 icons directly in the jgraph/drawio GitHub repo.

Usage:
  # Dump full catalog (run once, commit result as azure2-complete-catalog.txt):
  python3 search_azure2_icons_github.py --max-results 9999 > ../references/azure2-complete-catalog.txt

  # Search by keyword:
  python3 search_azure2_icons_github.py --search gateway

  # Validate matched icons resolve:
  python3 search_azure2_icons_github.py --search gateway --validate
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fallback search for Azure2 icons directly in jgraph/drawio GitHub repo"
    )
    parser.add_argument(
        "--search", nargs="*", default=[], help="Keywords to match in icon path"
    )
    parser.add_argument(
        "--max-results", type=int, default=50, help="Maximum results to print"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate each matched icon URL"
    )
    parser.add_argument(
        "--raw-base", default=RAW_BASE, help="Raw base URL for validation"
    )
