...
- Purpose: Fail CI when generated FastAPI OpenAPI schema drifts from the committed baseline without an intentional snapshot update.
- Contract baseline: `backend/openapi.snapshot.json` is generated by `backend/export_openapi.py`.
- CI gate: `backend/check_openapi_contract.py` compares the exported schema artifact against the committed snapshot after OpenAPI export.
- Frontend client drift gate: the `openapi-client-drift` CI job downloads the backend `openapi-schema` artifact, regenerates `frontend/src/generated/api-types.d.ts` and `frontend/src/generated/api-paths.js`, then fails on `npm run check:api-drift` when committed generated artifacts are stale.
