...
**Release Note:** Azure OpenAI production traffic is consolidated into West Europe with `gpt-4.1` primary and `gpt-4o` fallback; the Foundry benchmark plan keeps production routing unchanged until live gates pass, the Sweden Central one-region plan requires a parallel build with isolated Terraform state before any traffic shift, CI now enforces generated IaC validation, Checkov policy-as-code for checked-in Azure Terraform, CodeQL SAST, Vite client-env exposure guardrails, Docker Node base-image pinning, quarterly mutation-score baselines, and PostgreSQL plus pgvector Alembic migration smoke, service catalog refresh and rollback health verification use authenticated `/api/health`, infrastructure import mutations are authenticated/rate-limited/10 MB capped, deployment execution routes require admin bearer sessions, artifact/session mutation routes are API-key gated, one-time export capabilities are consumed only after successful artifact generation, production/staging JWT configuration fails closed without a configured secret, the Front Door WAF rate-limit rule applies to all IPv4 clients, SWA cookie-auth mutations use Strict SameSite CSRF protection, SWA consent state is recognized from `/.auth/me`, sign-in redirects preserve path/query/hash state, the rollback runbook documents Container Apps, ACR, Static Web Apps, and Alembic caveats, and P2 operational trust hygiene now covers consistent 429 throttling, SSE heartbeat regression coverage, frontend error reporting, production App Insights sampling, Roadmap modal stability, retryable analysis failures, IaC chat history trimming, vision cache sizing, tab-level ErrorBoundary recovery, token-based dark-mode consistency, LoginModal keyboard focus-order E2E coverage, pytest no-rerun flake tracking, mobile sign-in accessibility, authenticated user-menu keyboard semantics, guided-question toggle semantics, profile form accessibility, tokenized risk indicators, deploy coming-soon accessibility, and dialog focus restoration.
