...
| Cost estimate JSON | Backend Master | `cd backend && .venv/bin/python -m pytest tests/test_api.py::test_architecture_package_and_cost_outputs -q` validates currency, service rows, and total monthly estimate | No snapshot baseline | Smoke validates USD currency and service rows | `aws-hub-spoke` sample | `architecture-package-smoke-<run id>/raw/08-cost.json` | #703 |
| Cost CSV | Backend Master | No dedicated backend CSV contract test yet; smoke is the current release gate | No CSV snapshot baseline | Smoke checks `Service,` header and `TOTAL` row | `aws-hub-spoke` sample | `architecture-package-smoke-<run id>/artifacts/cost-estimate.csv` | #703 |
