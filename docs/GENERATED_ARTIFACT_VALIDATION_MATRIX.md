...
| Cost estimate JSON | Backend Master | `cd backend && .venv/bin/python -m pytest tests/test_cost_optimizer.py tests/test_sample_session_recreate.py -q` | No snapshot baseline | Smoke validates USD currency and service rows | `aws-hub-spoke` sample | `architecture-package-smoke-<run id>/raw/08-cost.json` | Covered for release smoke |
| Cost CSV | Backend Master | Cost endpoint coverage plus smoke CSV assertions | No CSV snapshot baseline | Smoke checks `Service,` header and `TOTAL` row | `aws-hub-spoke` sample | `architecture-package-smoke-<run id>/artifacts/cost-estimate.csv` | Covered for release smoke |
