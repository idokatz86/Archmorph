...
def _assert_conformance(*, code: str, iac_format: str, controls: list[str]) -> list[str]:
    ...
    for control in controls:
        if control == "required_tags":
            ...
            def matcher(key: str):
                if iac_format == "terraform":
                    return re.search(rf"\b{re.escape(key)}\s*=\s*\"", code, re.IGNORECASE)
                return re.search(rf"\b{re.escape(key)}\s*:\s*['\"]", code, re.IGNORECASE)
...
def test_generated_iac_matches_golden_secure_fixtures(fixture_path: Path, monkeypatch):
    ...
    try:
        ...
        assert terraform_resp.status_code == 200, terraform_resp.text
        assert bicep_resp.status_code == 200, bicep_resp.text
        terraform_code = terraform_resp.json()["code"]
        bicep_code = bicep_resp.json()["code"]
        assert fixture["terraform"].rstrip() in terraform_code.rstrip()
        assert fixture["bicep"].rstrip() in bicep_code.rstrip()

        controls = fixture["required_controls"]
        assert _assert_conformance(code=terraform_code, iac_format="terraform", controls=controls) == []
        assert _assert_conformance(code=bicep_code, iac_format="bicep", controls=controls) == []
    finally:
        try:
            SESSION_STORE.delete(diagram_id)
        except Exception:
