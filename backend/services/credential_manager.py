...
def store_credentials(session_token: str, provider: str, creds: dict) -> None:
    ...
    logger.info("Encrypted credentials stored for session_token ending in %s", session_token[-4:])
...
def clear_credentials(session_token: str) -> None:
    ...
    logger.info("Credentials cleared for session terminating in %s", session_token[-4:])
