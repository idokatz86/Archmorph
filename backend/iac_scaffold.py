...
from alz_profile import build_alz_profile
...
variable "owner" {
  description = "Workload owner tag value"
  type        = string
...
variable "criticality" {
  description = "Business criticality tag value"
  type        = string
  default     = "standard"
}
"""
...
def _generate_env_main(env: str, categories_with_services: Dict[str, List[dict]], project_name: str, region: str) -> str:
    ...
    for cat, svcs in categories_with_services.items():
        ...
        lines.append('  cost_center         = var.cost_center')
        lines.append('  data_classification = var.data_classification')
        lines.append('  criticality         = var.criticality')
        lines.append("}")
        lines.append("")
...
def _generate_env_tfvars(env: str, project_name: str, region: str) -> str:
    ...
    default_criticality = "mission-critical" if env == "prod" else "standard"
    lines = [
...
def generate_scaffold(
    ...
    files["terraform/alz-profile.json"] = json.dumps(
        build_alz_profile(analysis),
        indent=2,
        sort_keys=True,
    ) + "\n"
