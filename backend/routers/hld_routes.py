...
async def export_migration_package(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        lifecycle_receipt_payload = diagrams_compat._attach_lifecycle_receipt({}, diagram_id)
        trust_receipt = lifecycle_receipt_payload.get("trust_receipt")
        if trust_receipt:
            zf.writestr("analysis/trust-receipt.json", json.dumps(trust_receipt, indent=2))
...
- `analysis/` — Analysis summary, trust receipt, and cost estimate
...
"""
    ...
    record_event("trust_receipt_included_in_migration_package", {"diagram_id": diagram_id})
