...
async def generate_hld_endpoint(request: Request, diagram_id: str, _auth=Depends(verify_api_key)):
    ...
    try:
        ...
        logger.exception("HLD generation failed: %s", sanitize_log(e))  # codeql[py/log-injection] Handled by custom sanitize_log
...
async def _ensure_hld(session: dict, diagram_id: str) -> dict:
    ...
    try:
        ...
        session["hld_markdown"] = markdown
        SESSION_STORE[diagram_id] = session
        logger.info("Auto-generated HLD for session %s", sanitize_log(diagram_id))  # codeql[py/log-injection] Handled by custom sanitize_log
    except Exception as e:
        logger.warning("Auto-HLD generation failed for %s: %s", sanitize_log(diagram_id), sanitize_log(e))  # codeql[py/log-injection] Handled by custom sanitize_log
...
async def export_hld_endpoint(request: Request, diagram_id: str, _auth=Depends(verify_api_key)):
        ...
        if not diagram_b64 and session.get("mappings"):
            try:
                ...
                if arch_result and arch_result.get("content"):
                    ...
                    logger.info("Auto-generated architecture diagram for customer export of %s", sanitize_log(diagram_id))  # codeql[py/log-injection] Handled by custom sanitize_log
            except Exception as e:
                logger.warning("Failed to auto-generate architecture diagram: %s", sanitize_log(e))  # codeql[py/log-injection] Handled by custom sanitize_log
    ...
    try:
        ...
        logger.error("HLD export failed: %s", sanitize_log(e))  # codeql[py/log-injection] Handled by custom sanitize_log
...
async def _run_hld_job(job_id: str, diagram_id: str) -> None:
    ...
    try:
        ...
        logger.error("Async HLD generation failed: %s", sanitize_log(exc), exc_info=True)  # codeql[py/log-injection] Handled by custom sanitize_log
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if hld:
            try:
                ...
                logger.warning("DOCX export failed in package: %s", sanitize_log(exc))  # codeql[py/log-injection] Handled by custom sanitize_log
