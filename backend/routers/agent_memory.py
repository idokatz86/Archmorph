...
def list_documents(agent_id: str, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
...
def delete_document(agent_id: str, doc_id: str, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    _get_agent_verified(agent_id, db, user)
...
def list_episodes(agent_id: str, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    _get_agent_verified(agent_id, db, user)
...
def list_entities(agent_id: str, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    _get_agent_verified(agent_id, db, user)
    entities = db.query(AgentEntityMemory).filter(AgentEntityMemory.agent_id == agent_id).all()
    return entities

@router.delete("/")
def clear_memory(agent_id: str, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    _get_agent_verified(agent_id, db, user)
