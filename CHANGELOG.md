...
- **Secure icon-pack ingestion** — `POST /api/icon-packs` and `DELETE /api/icon-packs/{pack_id}` now require API-key auth, production-like deployments fail closed without `ARCHMORPH_API_KEY`, uploaded SVGs are sanitized, reserved built-in pack IDs are blocked for custom uploads, built-in provider catalogs are protected from custom-capacity eviction, and Excalidraw/draw.io/Visio plus landing-zone icon caches use invalidation/generation guards to avoid stale assets.
