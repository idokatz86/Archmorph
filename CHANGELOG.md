...
- **Sensitive access-control hardening** — production/staging JWT fallback now fails closed when `JWT_SECRET` is missing, deployment execution routes require an admin bearer session in addition to feature flags, generated artifact/session routes consistently require API-key or scoped export capability checks, successful exports consume one-time capabilities only after artifacts are produced, drift baseline routes require authentication/rate limits with valid error envelopes, and the Front Door WAF rate-limit rule now matches all IPv4 clients without an accidental negation.
