...
- **Canonical rollout priority, provenance, and compatibility service (#1237)** — replaced replaceable GitHub pending-slot serialization with renewable private Azure Blob ownership and deterministic rollback intents; bound Container Apps/Helm images to GitHub attestation, exact OCI labels/platform, source workflow/run, and embedded schema contracts; made the 013/014 bridge preserve canonical authenticated tenant-scoped core reads while denying all writes/effectful GETs with maintenance semantics; added customer-degraded paging; enforced exact 63-character Container Apps revision identities and complete readiness evidence; and removed the public Blob network cutover escape hatch.
- **Canonical release rollback and recovery evidence (#1237)** — now emits an HMAC-signed immutable final manifest for every successful backend release; manual rollback selects an exact final run/attempt or retained signed manifest and proves image/source/schema compatibility at zero traffic; Helm separates migration from workload rollout with compatible-prior or bridge protection and no schema-crossing atomic rollback; Kubernetes release Leases use renewable resourceVersion CAS with stale-holder takeover; and frontend recovery bundles validate their complete dist/API graph before atomic publication.
- **Canonical rollout supervision and migration truthfulness (#1237)** — made Container Apps traffic verification ignore only validated inert zero-weight entries while failing on every positive-route mismatch; persisted signed exact migration execution state for restart-safe stop-and-quiesce recovery; made rollout telemetry bounded, local-first, and non-gating; pinned Alembic upgrades to the reviewed reachable target; added explicit empty-database-only bootstrap; reframed downgrade CI as structural compatibility with populated-data refusal; and removed raw application-log streaming from release workflows.
- **Canonical erasure fixed-point hardening (#1237)** — made usage telemetry files cross-process locked and crash-safe with atomic replacement, fsync, lossless aggregate merges, and ETag-conditional Blob updates; fenced process-local model-response/version caches with durable purge tombstones at startup and every access; and replaced circular GET-effect coverage with runtime/OpenAPI route metadata plus independent AST regressions while removing replay, cost-insight, and project-analysis read effects.
- **Canonical durable analysis state CISO remediation (#1237)** — routed authenticated analysis/review/HLD/IaC/version mutations through one PostgreSQL-first UoW, linked generated artifacts to immutable versions, added version-aware Redis CAS and durable uniqueness/retry, introduced guarded `default_tenant` rehome auditing plus opaque SWA provider-subject scopes, protected restore namespaces with principal-bound capabilities and same-404 denials, and split anonymous `/healthz` liveness from required PostgreSQL/Redis `/readyz` readiness across Terraform and Helm.
- **Restart-safe accepted async jobs (#1239)** — analysis, IaC, and HLD submissions now persist versioned execution envelopes and atomic input/configuration hashes before returning `202`; Redis-backed workers use claim leases, heartbeats, bounded retries, periodic recovery, ownership-safe progress/finalization, startup admission-counter rebuilds, SSE recovery continuity, chaos coverage, and Azure Monitor recovery/backlog alerts.
- **#1240 release and public metadata truth** — established root `VERSION` as the generated/checkable product-version source, aligned backend/frontend/npm/badge/PRD/OpenAPI signals, kept Azure Landing Zone explicitly Beta because its GA/release issues were superseded rather than completed, removed source-code production CORS defaults, moved Terraform backend/legacy stack identifiers to private CI configuration, and added a redaction-safe public metadata lint.
- **#1241 truthful export artifact contracts** — ExportHub no longer advertises a fake Cost PDF or synthesizes a three-phase timeline from cost data. Cost CSV, seven-phase timeline JSON/Markdown/CSV, and the streamed analysis-report PDF now come from their canonical backend routes, rotate one-time export capabilities, and normalize into a shared filename/MIME/format/provenance contract before download. The compact package panel is also labeled accurately.
...
- **Python runtime container supply-chain hardening** — required a patched setuptools during image construction and removed `pip`, setuptools, and wheel from both final runtime images so pip's vulnerable vendored MessagePack and setuptools inventories cannot remain in production containers.
- **Fix production private network reachability for Redis and Blob Storage** — addressed a configuration gap where production Redis and Blob Storage could be deployed with deny-by-default access controls but no guaranteed connectivity path from the Container Apps runtime. Changes: (1) `enable_redis_private_endpoint` now defaults to `true`; `enable_storage_private_endpoint` variable added (defaults to `true`). (2) Storage private endpoint resources added (DNS zone, VNet link, private endpoint). (3) `Microsoft.Storage` service endpoint added to the container_apps subnet; container_apps subnet included in storage `virtual_network_subnet_ids` for prod. (4) Storage `public_network_access_enabled` set to `false` in prod. (5) `lifecycle { precondition {} }` blocks added to both `azurerm_redis_cache.main` and `azurerm_storage_account.main` to fail-fast in `terraform plan` when prod is configured without a private endpoint. (6) New Checkov policies `CKV_ARCHMORPH_4` (Redis no public access) and `CKV_ARCHMORPH_5` (Storage bypass list) prevent future deny-without-path regressions. (7) Health endpoint now performs an actual blob service reachability probe (`get_service_properties`) instead of only checking if the URL is configured, and Redis connection failures are labelled `unreachable` for operator clarity.

- **AI/export production guardrails** — bounded per-worker OpenAI admission (derived from deployment quota), Retry-After-aware backend/frontend retries for throttling and 5xx responses, corrected session-store byte accounting under overwrite/expiry, and direct vision token/cost metering with CostMeter durable bootstrap from persisted records.
- **#607 Azure OpenAI regional cutover** — provisioned a parallel OpenAI account, deployed the primary/fallback models with matching runtime names, granted the Container App managed identity the least-privilege inference role, updated private deployment configuration, and routed traffic to a healthy regional revision while retaining the prior account for rollback and zero-traffic verification.
...
- **Operational trust baseline (#851 #852 #864 #904)** — standardized retryable OpenAI throttling on 429 + `Retry-After`, added SSE idle-heartbeat regression coverage, introduced a frontend error reporter used by the ErrorBoundary/API/deploy paths, and lowered production Application Insights sampling to a configurable 10% default with matching OTel trace sampling.
- **Post-consent auth state** — SWA sign-in now keeps the user authenticated after consent even when `/api/auth/me` cannot see the forwarded SWA principal header, while still preferring the backend user profile whenever it is available.
- **Frontend auth and tab continuity (#810 #812 #813)** — sign-in redirects preserve path/query/hash state, the mobile Sign In trigger exposes a stable dialog-accessible name, and the main tab ErrorBoundary resets when users switch tabs so one failed view does not block navigation.
- **Deploy tab coming-soon accessibility (#827)** — replaced the greyscaled, `aria-hidden` deploy preview with a non-interactive status state so keyboard and screen-reader users no longer land on disabled preview controls.
- **User menu keyboard semantics (#823)** — authenticated user dropdown now uses menu/menuitem roles, advertises `aria-haspopup="menu"`, supports Arrow/Home/End roving focus, and closes on Escape with focus restored to the trigger.
- **Guided questions toggle semantics (#826)** — separated question-scope and expert-mode icon language, and exposed both header controls as switches with stable accessible names and checked state.
- **Profile form accessibility (#824)** — profile settings now use shared labeled form primitives so display name, company, role, source-cloud, and IaC-format controls are programmatically associated with their labels.
- **Risk color token accessibility (#825)** — HLD risk/WAF indicators and profile status messages now use theme tokens with icon-plus-label states instead of raw red/yellow/green color-only cues.
- **Theme-token dark-mode consistency (#809)** — removed the remaining unbound Tailwind `dark:` utility from the active frontend path and added LoginModal, DeployPanel, and GuidedQuestions regression coverage for token-based theming.
...
- **Sensitive access-control hardening** — production/staging JWT configuration fails closed when `JWT_SECRET` is missing and the deploy workflow no longer falls back to `ADMIN_KEY`; Container Apps now receive distinct admin/API secrets and secretref-backed ACS connection strings; legacy deployment execution and AI suggestion review routes require admin bearer sessions; generated artifact/session, chat, version/diff, share-management, and cost-insight routes require API-key or scoped export capability checks; successful exports consume one-time capabilities only after artifacts are produced; drift baseline routes require authentication/rate limits with valid error envelopes; and the Front Door WAF rate-limit rule now matches all IPv4 and IPv6 clients without an accidental negation.
...
- **Generated IaC security conformance fixtures** — expanded `tests/test_iac_output_validation.py` with golden AWS/GCP/mixed fixture coverage and negative-control fixtures that must fail semantic policy checks for managed identity, TLS, diagnostics, private connectivity, Key Vault handling, WAF/public ingress posture, required tags, and hardcoded-secret bans.
...
Resource-group hygiene pass on the live stack. Three orphaned or duplicated Azure resources were retired without service impact; concrete inventory remains in private operator evidence:

- **Stale App Service and plan** — deleted after confirming production traffic was served by Container Apps.
- **Duplicate Container Registry** — images were imported into the primary registry, the target revision passed health and traffic checks, and only then was the legacy registry removed.
- **Mis-grouped Cognitive Services account** — moved to its owning project resource group and verified at source and destination despite a misleading control-plane response.
