...
- **IaC Chat assistant** — interactive GPT-4o assistant for code modifications with compact history prompts that avoid replaying full prior code blobs
...
- **Scoped API keys and admin gates** — current access control focuses on distinct API/admin secrets, admin bearer-session checks, one-time export capabilities, feature flags, and guarded scaffold execution; deployment execution and AI suggestion review routes require admin bearer sessions, artifact/session read-write routes require API-key or scoped capability checks, and multi-tenant org/profile surfaces are no longer active routes
...
- **Zero Trust WAF** — Azure Front Door Premium with OWASP CRS 3.2 and all-client IPv4/IPv6 per-IP rate limiting
